    ...
    public function execute(): void {
        ...
        foreach ($recordset as $oldtopcategory) {
            ...
            try {
                ...
                $this->log_message('tasklogprocessingcategory', [
                    'topcategoryid' => $oldtopcategory->id,
                    'contextid' => $oldtopcategory->contextid,
                ...
                if (!$DB->record_exists('question_categories', ['parent' => $oldtopcategory->id])) {
                    ...
                    $trans->allow_commit();
                    $cleanedup++;
                    $this->log_message('tasklogdeletedemptycategory', $oldtopcategory->id);
                    continue;
                }
                ...
                if (!$newmod = question_bank_helper::get_default_open_instance_system_type($course)) {
                    ...
                    $this->log_message('tasklogcreateddestination', [
                        'cmid' => $newmod->id,
                        'courseid' => $course->id,
                        'topcategoryid' => $oldtopcategory->id,
                    ]);
                ...
                $transferred++;
                $this->log_message('tasklogtransferredcategory', [
                    'topcategoryid' => $oldtopcategory->id,
                    'count' => count($movedcategories),
                ]);
            } catch (\Throwable $e) {
